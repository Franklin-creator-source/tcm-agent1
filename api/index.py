...
# ═══════════════════════════════════════════════
# 强约束规则：追问、不编造、承认未知（笔记原版逻辑，仅修复 symptom_keywords）
# ═══════════════════════════════════════════════
...
⚠️ This response is for academic exchange only and does not constitute medical advice."""
    ...
    # ── 规则 3：追问 ──
    has_followup = any(kw in answer for kw in [
        "可能性", "可能是", "请补充", "建议您补充", "需要您补充",
        "possibility", "possible", "please provide", "please supplement"
    ])

    # 【修复】symptom_keywords 按语言取值，避免 bool 逻辑 bug
    if zh:
...
7. 出血病人要在使用活血化瘀药物时要选择活血止血的药物。脑出血病人使用活血化瘀药物需要在出血后24小时至72小时后发现血肿没有扩大才能从小剂量开始使用。
...
# ── 根路径路由：提供 index.html 静态文件（适配 Vercel rewrite） ──
